...
const validation: TemplateExtraValidation = (def) => {
  for (const microfrontend of def.microfrontends) {
    if (
      microfrontend.frontendPermissions &&
    ...
    if (microfrontend.apiProxies && Object.keys(microfrontend.apiProxies).length > 0) {
      if (
        Object.values(microfrontend.apiProxies).find(
          (proxy) => typeof proxy === 'object' && proxy.security && Object.keys(proxy.security).length > 0
        )
      ) {
        console.warn(
          colors.yellow(
            `OMG: Some APIs require security: Mashroom does not support that automatically, you have to add an appropriate proxy interceptor plugin manually.`
          )
        );
      }
    }
