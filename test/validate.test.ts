...
describe('validate', () => {
    ...
    it('fails if the definition file is not schema compliant', async () => {
        ...
        expect(errors).toBe('Schema validation failed: /microfrontends/0 must have required property \'globalLaunchFunction\'');
    });
    ...
    it('fails with invalid external schema references', async () => {
        ...
        expect(errors.startsWith('Message Schema of Microfrontend My First Microfrontend Topic \'ping\' is not valid:')).toBeTruthy();
    });
    ...
    it('fails with an invalid config schema', async () => {
        ...
        expect(errors).toBe('Config Schema of Microfrontend My First Microfrontend is not valid: strict mode: unknown keyword: "typex"');
    });
